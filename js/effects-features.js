...
export function applyEyebrowCleanup(state, dom, data) {
    ...
    const leftBrowMask = buildPolygonMask(state, dom, LEFT_EYEBROW, 0).data;
    const rightBrowMask = buildPolygonMask(state, dom, RIGHT_EYEBROW, 0).data;
    ...
    const TAIL_DARK_LUMA_THRESHOLD = 95;

    for (let y = 0; y < TARGET_H; y++) {
        for (let x = 0; x < TARGET_W; x++) {
            const i = (y * TARGET_W + x) * 4;

            const browMaskA = Math.max(leftBrowMask[i], rightBrowMask[i]) / 255;
            ...
            // 눈썹 내부의 옅은 부분만 후보로 사용
            // 너무 검정/진한 회색에 가까운 부분은 제외
            if (
                gBrow > 0.08 &&
                eyeProtect < 0.55 &&
                lum > DARK_LUMA_THRESHOLD &&
                originalLum > DARK_LUMA_THRESHOLD
            ) {
            ...
            if (
                tailMask > 0.05 &&
                lum > TAIL_DARK_LUMA_THRESHOLD &&
                originalLum > TAIL_DARK_LUMA_THRESHOLD
            ) {
